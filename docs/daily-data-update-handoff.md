...
- ⚠️ **structure 页面需要 qhkch VIP 登录态**（ 环境变量，fetch_url 已支持），无 cookie 时除国泰君安（免费样本）外全部「无权访问」。**待用户提供 Cookie 后每日流程需设置  再跑 fetch_seat_flow.py，否则外资组与大部分内资席位缺失**。国泰君安数据与用户截图逐项吻合（沪金 +10.47亿等）。
- 提交：私有仓 、公开仓 。
...
- 截图转录流程：同花顺 4 张 → `scripts/transcribe_ths_20260911.py`（原始字符串 + 单位换算）→ AKShare 对账修正 5 处（AL/I/NR/LC 收盘、BU 持仓 10 倍错位）→ 重生成 CSV。
- ⚠️ **事故记录与铁律**：本地 build 只产出当日快照（历史快照源文件不落地），直接把本地 `data/dashboard.json` 推公开仓会把 30 天历史清成 2 天（本次已从公开仓 git HEAD~1 恢复）。**今后发布 dashboard.json 前必须核对快照日期数 ≥ 现网 dates 数；本地只有新日期时，用 `git show HEAD~1:data/dashboard.json` 恢复全量后注入新快照再提交。**
- 东财技术面接口当日 0/6 异常，按缺失处理；OpenVLab 截图分辨率不足未转录，期权因子标缺失。
- 提交：私有仓 `3f360c2`（数据+转录脚本）、公开仓 `f0fa76d`（发布）+ `2adc227`（历史恢复修复）。
