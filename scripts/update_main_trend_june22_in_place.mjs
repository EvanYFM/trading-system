...
const outputDir = "output/main_trend_update_20260622";

const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(workbookPath));
const sheet = workbook.worksheets.getItemAt(0);
const values = sheet.getUsedRange().values;

const targetSerial = 46195; // 2026-06-22
const header = values[1];
const targetCol = header.findIndex((value) => Number(value) === targetSerial);
if (targetCol < 0) throw new Error("Could not find 2026-06-22 column.");

const targetAddress = sheet.getRangeByIndexes(0, targetCol, 1, 1).address.replace(/\d+$/, "");
if (targetAddress !== "FC") {
  throw new Error(`2026-06-22 is in ${targetAddress}, expected FC.`);
}
...
const preview = await workbook.render({ sheetName: sheet.name, range: "FC1:FC57", scale: 2, format: "png" });
await fs.writeFile(`${outputDir}/fc_column_preview.png`, new Uint8Array(await preview.arrayBuffer()));
