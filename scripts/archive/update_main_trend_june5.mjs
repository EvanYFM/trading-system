...
const inputPath = "C:/Users/29266/Desktop/主力趋势-6.5.xlsx";
const outputPath = "C:/Users/29266/Desktop/主力趋势-6.5-已更新.xlsx";
const outputDir = "output/main_trend_update_20260605";
...
const targetSerial = 46178; // 2026-06-05
const header = values[1];
const targetCol = header.findIndex((value) => Number(value) === targetSerial);
if (targetCol < 0) throw new Error("Could not find 2026-06-05 column.");

const targetAddress = sheet.getRangeByIndexes(0, targetCol, 1, 1).address.replace(/\d+$/, "");
if (targetAddress !== "EP") {
  throw new Error(`2026-06-05 is in ${targetAddress}, expected EP.`);
}
...
const preview = await workbook.render({ sheetName: sheet.name, range: "EP1:EP57", scale: 2, format: "png" });
await fs.writeFile(`${outputDir}/ep_column_preview.png`, new Uint8Array(await preview.arrayBuffer()));
