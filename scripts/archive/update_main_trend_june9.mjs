...
const inputPath = "C:/Users/29266/Desktop/主力趋势-6.8-已更新.xlsx";
const outputPath = "C:/Users/29266/Desktop/主力趋势-6.9-已更新.xlsx";
const outputDir = "output/main_trend_update_20260609";

const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(inputPath));
const sheet = workbook.worksheets.getItemAt(0);
const values = sheet.getUsedRange().values;

const targetSerial = 46182; // 2026-06-09
const header = values[1];
const targetCol = header.findIndex((value) => Number(value) === targetSerial);
if (targetCol < 0) throw new Error("Could not find 2026-06-09 column.");

const targetAddress = sheet.getRangeByIndexes(0, targetCol, 1, 1).address.replace(/\d+$/, "");
if (targetAddress !== "ER") {
  throw new Error(`2026-06-09 is in ${targetAddress}, expected ER.`);
}
...
const preview = await workbook.render({ sheetName: sheet.name, range: "ER1:ER57", scale: 2, format: "png" });
await fs.writeFile(`${outputDir}/er_column_preview.png`, new Uint8Array(await preview.arrayBuffer()));
