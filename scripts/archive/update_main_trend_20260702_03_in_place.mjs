...
const dateUpdates = [
  ...
  },
  {
    ...
    serial: 46206,
    notes: [
      "7.3 合成橡胶家人区徽章与文字方向不完全一致；按文字“大幅加空合成橡胶”解读为家人原始偏空，反向后记为家人+。",
    ],
    updates: new Map([
      ["鸡蛋", ["家人+", "内资+"]],
      ["玻璃", ["家人+", "内资+"]],
...
function repeatedConsecutiveItems(items, last3DateCols) {
  ...
  for (const [variety, rows] of byVariety.entries()) {
    ...
    if (current.length > best.length) best = current;
    if (best.length >= 2) {
      repeated.push({ variety, direction: best[0].direction, count: best.length, rows: best });
    }
  }
...
const allNotes = [];
const targetColumns = [];
for (const { date, serial, updates, notes } of dateUpdates) {
...
await fs.writeFile(
  ...
  JSON.stringify({ workbookPath, targetColumns, written: allWritten, missing: allMissing, notes: allNotes }, null, 2),
  "utf8",
);
...
await fs.writeFile(`${outputDir}/fg_fi_columns_preview.png`, new Uint8Array(await preview.arrayBuffer()));
