...
const outputDir = "output/main_trend_update_20260623";

const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(workbookPath));
const sheet = workbook.worksheets.getItemAt(0);
const values = sheet.getUsedRange().values;

const targetSerial = 46196; // 2026-06-23
const header = values[1];
const targetCol = header.findIndex((value) => Number(value) === targetSerial);
if (targetCol < 0) throw new Error("Could not find 2026-06-23 column.");

const targetAddress = sheet.getRangeByIndexes(0, targetCol, 1, 1).address.replace(/\d+$/, "");
if (targetAddress !== "FD") {
  throw new Error(`2026-06-23 is in ${targetAddress}, expected FD.`);
}
...
const preview = await workbook.render({ sheetName: sheet.name, range: "FD1:FD57", scale: 2, format: "png" });
await fs.writeFile(`${outputDir}/fd_column_preview.png`, new Uint8Array(await preview.arrayBuffer()));
