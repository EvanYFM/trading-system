...
const inputPath = "F:/codex项目/output/main_trend_update_20260610/主力趋势-6.10-已更新.xlsx";
const outputDir = "output/main_trend_update_20260611";
const outputPath = `${outputDir}/主力趋势-6.11-已更新.xlsx`;

const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(inputPath));
const sheet = workbook.worksheets.getItemAt(0);
const values = sheet.getUsedRange().values;

const targetSerial = 46184; // 2026-06-11
const header = values[1];
const targetCol = header.findIndex((value) => Number(value) === targetSerial);
if (targetCol < 0) throw new Error("Could not find 2026-06-11 column.");

const targetAddress = sheet.getRangeByIndexes(0, targetCol, 1, 1).address.replace(/\d+$/, "");
if (targetAddress !== "ET") {
  throw new Error(`2026-06-11 is in ${targetAddress}, expected ET.`);
}
...
const preview = await workbook.render({ sheetName: sheet.name, range: "ET1:ET57", scale: 2, format: "png" });
await fs.writeFile(`${outputDir}/et_column_preview.png`, new Uint8Array(await preview.arrayBuffer()));
