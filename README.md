...
浏览器打开 `http://127.0.0.1:8788/`。当前 MVP 包含总览、历史日期切换、品种详情、品种全景和历史回看；总览中的强共振卡可直接进入对应品种详情。详情页展示东方财富期货主力合约的报告日收盘价与当日涨跌幅，并按样本席位汇总所有合约后列出净多、净空前五；样本不足五家时显示实际数量。趋势动物只提供趋势温度和强度，行情与趋势事实分列。页面只消费已生成底表，不改变三方计算逻辑。行情或趋势快照日期与报告日不一致时会明确标记为非当日。
...
沪银 `AG`、焦煤 `JM`、燃料油 `FU`、生猪 `LH`、碳酸锂 `LC`、鸡蛋 `JD` 已接入技术面详情：`scripts/fetch_eastmoney_technical_snapshot.py` 优先锁定报告日主力合约，日线只保留 MA5/20/60，并读取 15 分钟与 60 分钟成交量、持仓量和 K 线。分钟结构先处理包含关系，再识别分型、简化笔和最近中枢，统一输出“偏多 / 中枢震荡 / 偏空”；成交量与持仓量用于区分增仓推动和减仓推动，中枢边界与均线共同生成最近支撑压力。品种详情顶部使用“品种下拉 + 可选搜索 + 板块筛选”，当前 63 个商品均可进入详情；技术面仍只覆盖上述 6 个品种。技术指标为本地确定性计算，不消耗 LLM token；当前主力合约历史不等同于复权连续合约，简化缠论也不等同于严格背驰或买卖点确认。
