...
构建时会把每个交易日固化为独立 `snapshots/YYYYMMDD.json`，主页面只汇总快照，因此历史数据不会被次日覆盖。当前交易日自动化执行“抓取结构化数据 -> 写入每日快照 -> 构建并验证本地网页 -> 私有发布 Sites”；两份日报脚本保留为数据生产和对账层，但默认跳过 HTML。期货通和 OpenVLab Legend 截图未提供时明确保留缺失，不沿用旧截图；用户补发对应交易日截图后重建并再次私有发布同日快照。
