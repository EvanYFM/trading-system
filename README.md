...
沪银 `AG` 已加入技术面试点：`scripts/fetch_eastmoney_technical_snapshot.py` 优先锁定报告日主力合约，日线只保留 MA5/20/60，并读取 15 分钟与 60 分钟成交量、持仓量和 K 线。分钟结构先处理包含关系，再识别分型、简化笔和最近中枢，统一输出“偏多 / 中枢震荡 / 偏空”；成交量与持仓量用于区分增仓推动和减仓推动，中枢边界与均线共同生成最近支撑压力。技术指标为本地确定性计算，不消耗 LLM token；当前主力合约历史不等同于复权连续合约，简化缠论也不等同于严格背驰或买卖点确认。
